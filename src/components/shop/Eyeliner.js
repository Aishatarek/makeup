...
import eyeliners from "../../api/eyeliners.json";
import Eyelineritem from "./Eyelineritem";
import { Col, Container, Row } from "reactstrap";
import {Link} from "react-router-dom";
import Footer from "../Footer";
import Carsouleyeliner from "./Carsouleyeliner";
function Eyeliner() {
              ...
              <Link className="link1"  to="/Shopall">All</Link>
                ...
                <div className="inputs">
                  <form>
                    ...
                    <br />
                    <input type="radio" className="all" name="brand" value="Haus" onChange={event => { setSearch4(event.target.value) }} />
          ...
          <Col lg="9" md="9" sm="12">
            <div>
              <div className="shopsec1" className=" wow slideInRight"  data-wow-duration="2s" data-wow-delay="0.1s">
                ...
                {eyeliners.filter((eyeliner) => {
                  if (search === "" && search3 === "" && search4 === "") {
                    if (search2 == 0) { return eyeliner; }
                    else if (eyeliner.price == search2) {
                      return eyeliner;
                    }
                  }
                  else if (eyeliner.name.toLowerCase().includes(search.toLowerCase()) && eyeliner.color.includes(search3) && eyeliner.brand.toLowerCase().includes(search4.toLowerCase())) {
                    if (search2 == 0) { return eyeliner; }
                    else if (eyeliner.price == search2) {
                      return eyeliner;
                    }
                  }
                }).map((eyeliner) => (<Col lg="3" className=" wow slideInRight"  data-wow-duration="2s" data-wow-delay="0.1s"
                  md="3"
                  sm="12"
                  key={eyeliner.id} >
                  <Eyelineritem eyeliner={eyeliner} />
                </Col >))
                }
